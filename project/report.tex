...
$$
\text{performance}_i = c(i, S_{|S^i|}^i, S_0^i) + \sum_{t = 0}^{|S^i|-1} c(i, S_t^i, S_{t+1}^i)
$$
...
$$
c(i, \mathit{src}, \mathit{dst}) = \begin{cases}
...
where $\alpha, \beta \geq 1$, but there are no constraints on $\gamma$.
