...
$$
\mathit{cost}(S) =
  \left( \alpha \beta \cdot \mathit{cost}_\mathit{time}(S) \right) +
  \left( (1-\alpha) \cdot \mathit{cost}_\mathit{quality}(S) \right) +
...
$\beta$ is a normalization parameter that makes $\mathit{cost}_\mathit{time}$ directly comparable to $\mathit{cost}_\mathit{quality}$. Its calculation is discussed in the next section.
...
\beta =
    \left( 2 \cdot \max \left(
      ...
      \max_{r=1}^{n_\mathit{robots}} \max_{t=1}^{n_\mathit{tasks}} \mathit{task \, time}(r, t)
    \right) \right)^{-1}
$$
