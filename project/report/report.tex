...
The experiment run was to evaluate each algorithms five times over each dataset. The algorithms were run on a single MacBook Pro with a 2.6Hz quad-core Intel Core-i7 processor, and 8GB of RAM to ensure numeric consistency between the individual optimization processes.
...
The large scale data set was the most interesting and was the real differentiator between the algorithms. There was much more variation in convergence speed and final solutions when the algorithms were run against the large data set. Our more complex algorithms had much more trouble converging as a result of the size of the problem space. This is because some of our algorithms scale to large problem sets much more effectively than others. For example, our ACO implementation must do N calculations at each node, and there are N nodes to consider for each ant. TS for comparison only has to look at a random selection of the neighborhood to find an improving solution, or a solution not in the tabu list. 
...
We found that our algorithms had varied performance which is quite apparent in Figure 1. Simulated Annealing was our most successful algorithm due to a fast convergence rate with the most optimal solution. Simulated Annealing was known to be effective in solving problems similar to MRTA including Quadratic Assignment in previous papers\cite{Badreldin}, so this is not a particularly surprising discovery.

The more complex algorithms were the P-Metaheuristic algorithms:Ant Colony Optimization, Particle Swarm Optimization and Genetic Algorithm. None of them had incredible speed of convergence, though GA did find very strong solutions. These algorithms were the most difficult to implement and optimize so their relatively poor performance is expected. Each of these algorithms contains significantly more calculations per iteration and tuning parameters than their simpler alternatives.

This difference in scaling is one of our primary concerns for future study because with algorithms that scale more effectively we may have been able to get better results from our P-Metaheuristic algorithms. More optimizations are needed in order to implement these algorithms as effectively our S-Metaheuristic algorithms. Because we had only intermediate experience writing MATLAB programs we were not able to optimize our program as thoroughly as expert MATLAB programmers. The choice of language also made an impact on program speed, if we had implemented our algorithm in a faster language we may have seen a boost in performance, so this is an avenue of future study. Besides just optimizing program flow we also had to change the internal parameters of the algorithms in order to achieve better performance.
