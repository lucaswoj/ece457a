...
Observe that $S_{1,4}$ has the lowest cost, is not tabu, and improves upon our $S_\textit{best}$. Therefore, we let
$$S_{\textit{best}} = S_{1, \textit{best}} = S_{1,4} = [1, 0, 2, 3]$$
...
We begin with $$S_{1, \text{best}} = [1, 0, 2, 3]$$

Next, we find all neighboring solutions and evaluate the cost of each.
...
Observe that $S_{2,2}$ has the lowest cost. Although this solution is tabu, it is has a lower cost than $S_\textit{best}$, so we allow it to become $S_\textit{best}$ and $S_{2, \textit{best}}$.
