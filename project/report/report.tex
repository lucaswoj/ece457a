...
The last component that needs to be specified is the survivor selection algorithm. We chose to use a steady state Genetic Algorithm, so the entire generation is replaced by the children of the next generation if the child solution is an improvement on the parent from which it was derived. Because more children are generated than the size of the generation there is some selection pressure to choose better solutions, but because solutions are not kept from every iteration this allows us to diversify more than if we just chose the best solutions between both the children and parents as explained in course slides \cite{Talk}.
