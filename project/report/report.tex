...
In an MRS, there is heterogeneity in robot composition, as well as the tasks that are assigned to visit. This requires the augmentation of the ``salesman'' and ``city'' ideas to support some differentiating factors. This report has identified 5 main features:
...
\item \textbf{Priority}: Some tasks are more important than others and should be considered first.
...
In this section, we will consider five metaheuristics in the context of complex task allocation:
\begin{itemize}
\item tabu search
...
Another random number is generated: $r=0.65$, another crossover. This crossover will be between $S_{0,2}$ and $S_{0,4}$, with crossover points as 2 and 4. This yields two more children:
...
As is generally recommended when using PSO, we initialize the constants $c_1, c_2$ and $w$ as
\begin{align*}
c_1 = c_2 & = 1.4944 \\
w & = 0.792
\end{align*}
...
of a velocity by a scalar as defined above\cite{PermutationPSO}.
$$
v_{j,i} =
  w v_{j-1, i} +
...
Our ACO implementation had four parameters (separate from our cost function) needing to be tuned, $\alpha, \beta, \rho, r_0$, the exploitation constant, exploration constant, pheromone evaporation rate and likelihood-of-exploitation probability, respectively. These parameters form a pareto-optimality frontier, where none can be changed without negatively impacting the entire system, however, such a pareto-optimal solution is not unique. In addition, several regions of the parameter space prevented convergence entirely.
